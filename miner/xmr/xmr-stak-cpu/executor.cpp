...
void executor::on_pool_have_job(size_t pool_id, pool_job& oPoolJob)
{
	...
	uint64_t diff = pool->get_current_diff();
	...
	else
	{
		if(!pool->have_sock_error())
		{
			...
			if(strncasecmp(error.c_str(), "Unauthenticated", 15) == 0)
			{
				...
				ipc_event_queue->push(
					cauchy::on_miner_result(Result::REJECTED)
				);
			}
...
void executor::ex_main()
{
	...
	// Cauchy: Schedule event for reporting to client application.
